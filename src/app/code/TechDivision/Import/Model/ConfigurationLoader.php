...
/**
 * TechDivision\Import\Model\ConfigurationLoader
 *
 * NOTICE OF LICENSE
 *
...
use TechDivision\Import\Command\InputArgumentKeys;
use TechDivision\Import\Command\MagentoConfigurationKeys;
...
class ConfigurationLoader
{
     ...
     */
    public function __construct(DirectoryList $directoryList, $configurationFactoryClass)
    {
    ...
    public function load(InputInterface $input, $entityTypeCode)
    {
        ...
        // option, if yes override the value from the configuration file
        if ($installationDir = $input->getOption(InputOptionKeys::SYSTEM_NAME) &&
            $instance->getSystemName() === null
        ) {
            $instance->setSystemName($installationDir);
        }
